...
import type { DoseLog, InjectionSite, Membership, OnboardingTask, Plan, PlanId, PortalPatient, WeightEntry } from "./types";
...
const FULL_90_DAY: Membership["entitlements"] = ["book-doctor", "book-nurse", "book-dietitian", "book-health-coach", "prescriptions", "community", "content-library", "insurance-docs", "nurse-messaging"];
...
const SITES: InjectionSite[] = ["abdomen-left", "abdomen-right", "thigh-left", "thigh-right"];

/** weekly doses from `fromDay` (negative = past) for `weeks` weeks, skipping future dates */
const doses = (fromDay: number, weeks: number, doseMg: number[]): DoseLog[] => {
  const out: DoseLog[] = [];
  for (let i = 0; i < weeks; i++) {
    ...
    out.push({
      id: `dose-${fromDay}-${i}`,
      takenAtUtc: daysFromNow(d, 8, 0),
      doseMg: doseMg[Math.min(i, doseMg.length - 1)],
      site: SITES[i % SITES.length],
      sideEffects: i < 2 ? ["nausea"] : [],
    });
  }
  return out;
};
...
export const PORTAL_PATIENTS: PortalPatient[] = [
  // ---------------------------------------------------------------- Seán: consult_paid (paid an hour ago)
  {
    ...
    firstName: "Seán",
    lastName: "Murphy",
    membership: { state: "consult_paid", plan: PLANS["consult-89"], entitlements: ["nurse-messaging", "insurance-docs"] },
  ...
  // ---------------------------------------------------------------- Aoife: consult_done (day 12)
  {
    ...
    membership: {
      ...
      entitlements: ["prescriptions", "nurse-messaging", "insurance-docs", "content-library"],
    },
    ...
    medication: {
      ...
      currentDoseMg: 2.5,
      doseWeekday: 3,
      lastDoseUtc: daysFromNow(-4, 8, 0),
      titration: [
        { fromDay: 0, doseMg: 2.5, label: "Weeks 1–4" },
        { fromDay: 28, doseMg: 5, label: "Weeks 5–8" },
        { fromDay: 56, doseMg: 7.5, label: "Weeks 9–12" },
      ],
    },
    doses: doses(-11, 2, [2.5, 2.5]),
    onboarding: [
      task("consult", "Specialist consultation", "Completed with Dr Keogh", true),
      task("rx", "Medication delivered", "Pure Pharmacy — delivered by courier", true, { href: "/treatment" }),
  ...
  // ---------------------------------------------------------------- Ciarán: ninety_day_active (day 38)
  {
    ...
    medication: {
      ...
      currentDoseMg: 5,
      doseWeekday: 1,
      lastDoseUtc: daysFromNow(-2, 7, 45),
      titration: [
        { fromDay: -21, doseMg: 2.5, label: "Weeks 1–4" },
        { fromDay: 7, doseMg: 5, label: "Weeks 5–8" },
        { fromDay: 35, doseMg: 7.5, label: "Weeks 9–12" },
      ],
    },
    doses: doses(-58, 9, [2.5, 2.5, 2.5, 2.5, 5, 5, 5, 5, 5]),
    onboarding: [
  ...
  // ---------------------------------------------------------------- Margaret: legacy_member on Ongoing Care €75
  {
    ...
    membership: {
      ...
      nextChargeAmount: 75,
      entitlements: ["book-doctor", "book-nurse", "ad-hoc-sessions", "prescriptions", "community", "content-library", "nurse-messaging"],
    },
    ...
    medication: {
      ...
      currentDoseMg: 10,
      doseWeekday: 6,
      lastDoseUtc: daysFromNow(-1, 9, 0),
      titration: [{ fromDay: -150, doseMg: 10, label: "Maintenance" }],
    },
    doses: doses(-64, 10, [10]),
    onboarding: [
  ...
  // ---------------------------------------------------------------- Dara: ninety_day_overdue (instalment failed)
  {
    ...
    membership: {
      ...
      entitlements: ["prescriptions", "nurse-messaging", "content-library"],
    },
    ...
    medication: {
      ...
      currentDoseMg: 5,
      doseWeekday: 4,
      lastDoseUtc: daysFromNow(-13, 8, 0),
      titration: [
        { fromDay: -21, doseMg: 2.5, label: "Weeks 1–4" },
        { fromDay: 7, doseMg: 5, label: "Weeks 5–8" },
      ],
    },
    doses: doses(-69, 8, [2.5, 2.5, 2.5, 2.5, 5, 5, 5, 5]),
    onboarding: [
  ...
  // ---------------------------------------------------------------- Fiona: ninety_day_completed (day 92)
  {
    ...
    membership: {
      ...
      programmeDay: 93,
      entitlements: ["prescriptions", "community", "content-library", "nurse-messaging"],
    },
    ...
    medication: {
      ...
      currentDoseMg: 10,
      doseWeekday: 2,
      lastDoseUtc: daysFromNow(-3, 8, 0),
      titration: [
        { fromDay: -92, doseMg: 2.5, label: "Weeks 1–4" },
        { fromDay: -64, doseMg: 5, label: "Weeks 5–8" },
        { fromDay: -36, doseMg: 7.5, label: "Weeks 9–12" },
        { fromDay: -8, doseMg: 10, label: "Maintenance" },
      ],
    },
    doses: doses(-115, 17, [2.5, 2.5, 2.5, 2.5, 5, 5, 5, 5, 7.5, 7.5, 7.5, 7.5, 10]),
    onboarding: [
...
export function logDose(userId: string, dose: Omit<DoseLog, "id" | "takenAtUtc"> & { takenAtUtc?: string }): DoseLog {
  const p = store().get(userId);
  if (!p) throw new Error("no patient");
  const d: DoseLog = { id: `dose-${Date.now()}`, takenAtUtc: dose.takenAtUtc ?? new Date().toISOString(), ...dose };
  p.doses.push(d);
  if (p.medication) p.medication.lastDoseUtc = d.takenAtUtc;
  return d;
}
...
export function continueToOngoing(userId: string, plan: "ongoing-75" | "ongoing-150") {
  ...
  p.membership = {
    ...
    entitlements: ["book-doctor", "book-nurse", "ad-hoc-sessions", "prescriptions", "community", "content-library", "nurse-messaging", ...(plan === "ongoing-150" ? (["book-dietitian", "book-health-coach"] as const) : [])],
  };
